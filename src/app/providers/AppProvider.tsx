...
import { User, Send, DollarSign, History, MessageCircle, MapPin, Settings, FileText, Brain, CheckCircle, Truck, Clock, Package } from 'lucide-react';
import {
  Usuario,
  FormularioEnvioData,
  OpcionSidebar,
...
interface AppContextType {
  ...
  rolUsuario: 'cliente' | 'administrativo' | null;
  setRolUsuario: (value: 'cliente' | 'administrativo' | null) => void;
  usuarioActual: AuthUser | null;
  setUsuarioActual: (value: AuthUser | null) => void;
...
export const AppProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  ...
  const [rolUsuario, setRolUsuario] = useState<'cliente' | 'administrativo' | null>(null);
  const [usuarioActual, setUsuarioActual] = useState<AuthUser | null>(null);
  ...
  // Estados para formulario de envío
  const [formularioEnvio, setFormularioEnvio] = useState<FormularioEnvioData>({
    remitente: {
      nombre: '',
      telefono: '',
      direccion: '',
  ...
  const logout = () => {
    ...
    setOpcionSeleccionada('inicio'); // Volver al inicio
  };

  const checkAuth = async () => {
    try {
      const response = await AuthService.checkActiveSession();
      if (response.success && response.user) {
        setUsuarioActual(response.user);
        setRolUsuario(response.user.rol);
        setEstaLogueado(true);
      } else {
        logout();
      }
